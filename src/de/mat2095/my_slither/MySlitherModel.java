...
public class MySlitherModel {
    ...
    public synchronized void addSnake(int snakeID, String name, double x, double y, double wang, double ang, double sp, double fam, Deque<SnakeBodyPart> body) {
    ...
    public synchronized void removeSnake(int snakeID) {
        snakes.remove(snakeID);
    }

    public synchronized void setOwnSnakeWang(double wang) {
        wAngle = wang;
    }

    public synchronized void setOwnSnakeBoost(boolean boost) {
    ...
    public synchronized void addPrey(int id, double x, double y, double radius, int dir, double wang, double ang, double sp) {
    ...
    public synchronized void removePrey(int id) {
        preys.remove(id);
    }

    public synchronized void addFood(int x, int y, double size) {
        foods.put(y * gameRadius * 3 + x, new Food(x, y, size));
    }

    public Food getFood(int x, int y) {
        return foods.get(y * gameRadius * 3 + x);
    }
    ...
    public synchronized void addSector(int x, int y) {
        sectors[y][x] = true;
    }

    public synchronized void removeSector(int x, int y) {
        sectors[y][x] = false;
